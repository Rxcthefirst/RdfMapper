    ...
    >
      ...
      <Box sx={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', mb: 2 }}>
        <Box sx={{ flex: 1 }}>
          ...
          <Typography variant="subtitle2" color="text.secondary" gutterBottom>
            Column: <strong>{matchDetail.column_name}</strong>
          </Typography>
          <Typography variant="body2" color="text.secondary" sx={{ wordBreak: 'break-word' }}>
            Mapped to: <strong>{matchDetail.matched_property.split('#').pop()?.split('/').pop()}</strong>
          </Typography>
        </Box>
      ...
      <Paper elevation={2} sx={{ p: 2, mb: 3, bgcolor: 'background.default' }}>
        ...
        <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
          <Typography variant="h3" color={getConfidenceColor(matchDetail.confidence_score) + '.main'}>
            {(matchDetail.confidence_score * 100).toFixed(1)}%
          </Typography>
          <Stack spacing={0.5}>
            <Chip
              label={getMatchTypeLabel(matchDetail.match_type)}
              size="small"
              color={getConfidenceColor(matchDetail.confidence_score) as any}
              icon={<CheckCircleIcon />}
            />
            <Typography variant="caption" color="text.secondary">
              Primary: {matchDetail.matcher_name}
            </Typography>
          </Stack>
        </Box>
      </Paper>
      ...
      {hasAmbiguity && (
        <Alert severity="warning" icon={<WarningAmberIcon />} sx={{ mb: 3 }}>
          <Typography variant="body2">
            <strong>Ambiguity Detected:</strong> {matchDetail.ambiguity_group_size} properties had similar confidence scores.
            An ambiguity penalty was applied. Review alternates below.
          </Typography>
        </Alert>
      )}
                ...
                <TableBody>
                  {matchDetail.evidence.map((ev, idx) => (
                    <TableRow key={idx} hover>
                      ...
                      <TableCell>
                        <Chip
                          label={getMatchTypeLabel(ev.match_type)}
                          size="small"
                          variant="outlined"
                        />
                      </TableCell>
                      <TableCell align="right">
                        <Typography
                          variant="body2"
                          color={getConfidenceColor(ev.confidence) + '.main'}
                          sx={{ fontWeight: 600 }}
                        >
                ...
                <Box>
                  ...
                  {matchDetail.boosters_applied!.map((boost, idx) => (
                    <Box key={idx} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                      ...
                      <Typography variant="body2">
                        {boost.type.replace('_', ' ')}: <strong>+{(boost.value * 100).toFixed(1)}%</strong>
                      </Typography>
                    </Box>
                  ))}
                </Box>
                ...
                <Box>
                  ...
                  {matchDetail.penalties_applied!.map((penalty, idx) => (
                    <Box key={idx} sx={{ display: 'flex', alignItems: 'center', gap: 1, mb: 1 }}>
                      ...
                      <Typography variant="body2">
                        {penalty.type.replace('_', ' ')}: <strong>−{(penalty.value * 100).toFixed(1)}%</strong>
                      </Typography>
                    </Box>
                  ))}
                </Box>
            ...
            <Stack spacing={1} sx={{ mt: 2 }}>
              {matchDetail.alternates.map((alt, idx) => (
                <Paper
                  ...
                  onClick={() => {
                    if (onSwitchToAlternate) {
                      onSwitchToAlternate(matchDetail.column_name, alt.property);
                    }
                  }}
                >
                  <Box sx={{ flex: 1 }}>
                  ...
                  <Tooltip title="Combined confidence score">
                    <Chip
                      label={`${(alt.combined_confidence * 100).toFixed(1)}%`}
                      size="small"
                      color={getConfidenceColor(alt.combined_confidence) as any}
                    />
                  </Tooltip>
                </Paper>
              ))}
      ...
      <Alert severity="info" icon={<InfoOutlinedIcon />} sx={{ mt: 3 }}>
        <Typography variant="body2">
          <strong>How to interpret:</strong> The evidence stack shows all matchers that contributed to this decision.
          The final score combines the base confidence with boosters (e.g., type compatibility) and penalties (e.g., ambiguity).
        </Typography>
      </Alert>
    </Drawer>
  );
